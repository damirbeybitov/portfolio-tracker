...
OPTIONAL_COLUMNS = {"fx_rate_usd_kzt", "commission_usd", "split_ratio", "notes"}
...
class ImportService:
    ...
    def _parse_file(file_bytes: bytes, filename: str) -> list[dict]:
        ...
        if lower.endswith(".csv"):
            return ImportService._parse_csv(file_bytes)
        elif lower.endswith((".xlsx", ".xls")):
            return ImportService._parse_excel(file_bytes)
        else:
    ...
    def _parse_excel(file_bytes: bytes) -> list[dict]:
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail="Excel import requires 'openpyxl'. Add it to requirements.txt.",
            )
        ...
        missing = REQUIRED_COLUMNS - set(header_norm)
        if missing:
            raise HTTPException(
                status_code=422,
                detail=f"Missing required columns: {', '.join(sorted(missing))}",
    ...
    async def _row_to_transaction(db: AsyncSession, row: dict) -> TransactionCreate:
        ...
        if type_raw not in TransactionType.__members__:
            raise ValueError(
                f"Invalid type '{type_raw}'. Must be one of: {', '.join(TransactionType.__members__)}"
            )
        ...
        commission_usd = ImportService._parse_decimal(row.get("commission_usd"), default=Decimal("0"))

        fx_rate_raw = row.get("fx_rate_usd_kzt")
        fx_rate = ImportService._parse_decimal(fx_rate_raw, default=None) if fx_rate_raw not in (None, "") else None

        split_raw = row.get("split_ratio")
        split_ratio = ImportService._parse_decimal(split_raw, default=None) if split_raw not in (None, "") else None
    ...
    async def import_bank_transactions(
        ...
        rows = ImportService._parse_file(file_bytes, filename, required=BANK_REQUIRED_COLUMNS)
        ...
        return BankTransactionImportResult(total=imported + failed, imported=imported, failed=failed, results=results)
    ...
    def _row_to_bank_transaction(row: dict) -> BankTransactionCreate:
        ...
        return BankTransactionCreate(
            type=BankTransactionType(type_raw), date=tx_date, amount=amount,
            related_account_id=related_id, fx_rate=fx_rate, notes=notes,
        )